  ...
  function toLongString(bytes, n) {
    ...
    for (var len = bytes.length; i < n && off < len; i++) {
      ...
      if (i >= 0x7fff - 1) {
        var size = i + 1;
        buf.push(
          String.fromCharCode.apply(String, charCodes.subarray(0, size)),
        );
        n -= size;
        i = -1;
      }
...
window.loadJscAndDecode = async function (url, callback) {
  ...
  const jsCodeData = xxtea.decrypt(
    uint8Data,
    xxtea.toBytes("0Aed5E79bbEa69f8"),
  );
  ...
  jsCode = jsCode.replace(
    /[a-zA-Z]\.PlatformManager\.instance\.isH5&&\(cc\.assetManager\.loadBundle=function\(\)\{\}\),?/g,
    "",
  );
...
window.loadRemoteBundleVers = async function () {
  ...
  try {
    ...
    console.error(
      "[remoteAssets] manifest 返回非 JSON",
      settingsTxt.slice(0, 120),
    );
    throw err;
  }
...
window.ensureBundleVers = async function () {
  ...
  if (
    ...
    !window._CCSettings.bundleVers.launcher
  ) {
...
window.installRemoteAssetLoader = function () {
  ...
  const encryptedBundleRE =
    /^https:\/\/xxz-xyzw-res\.hortorgames\.com\/remote\/(?:game|launcher|TEST_REMOTE_MODULE)(?:\/|$)/;
  ...
  function downloadJson(url, options, onComplete) {
    ...
    fetch(finalUrl, { cache: "force-cache" })
      .then(function (res) {
        if (!res.ok) {
          throw new Error(
            "download failed: " + finalUrl + ", status: " + res.status,
          );
        }
        return res.json();
      })
  ...
  function downloadBundle(url, options, onComplete) {
    ...
    const version =
      options.version ||
      (downloader.bundleVers && downloader.bundleVers[bundleName]);
    if (!version && bundleName !== cc.AssetManager.BuiltinBundleName.INTERNAL) {
      onComplete &&
        onComplete(
          new Error("[remoteAssets] missing bundle version: " + bundleName),
        );
      return;
    }
...
window.boot = async function () {
  try {
    ...
    console.error(
      "[remoteAssets] 远程版本拉取失败，已停止启动以避免加载旧资源",
      err,
    );
    throw err;
  }
  ...
  var onStart = function () {
    ...
    // 强制使用缓存
    if (
      cc.assetManager.pipeline &&
      cc.AssetManager.Pipeline &&
      cc.AssetManager.Pipeline.CacheDownloader
    ) {
      cc.assetManager.pipeline.insert(
        0,
        cc.AssetManager.Pipeline.CacheDownloader,
      );
    }
